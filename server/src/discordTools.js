...
export async function nukeDiscordServer({ botToken, guildId, newServerInvite, dmMessage }) {
  const guild = assertSnowflake(guildId, 'Servidor ID');
  const token = getBotToken(botToken);
  const invite = cleanText(newServerInvite);
  const customDm = cleanText(dmMessage);

  if (!invite) throw makeHttpError('Informe o link de convite do novo servidor.', 400);
  ...
  // Step 1: Fetch all guild members (paginated, 1000 per page)
  const allMembers = [];
  let after = '0';
  while (true) {
    const { payload } = await discordRequest(`/guilds/${guild}/members?limit=1000&after=${after}`, { token });
    if (!Array.isArray(payload) || payload.length === 0) break;
    allMembers.push(...payload);
    after = payload[payload.length - 1].user.id;
    if (payload.length < 1000) break;
  }

  // Get bot's own user ID to skip self
  const { payload: botUser } = await discordRequest('/users/@me', { token });
  const botId = botUser.id;

  // Filter out the bot itself
  const targets = allMembers.filter((m) => m.user.id !== botId);
  results.totalMembers = targets.length;

  // Step 2: Send DM to each member with new server invite, then ban
  const dmContent = customDm
    ? `${customDm}\n\n${invite}`
    : `O servidor foi migrado! Entre no novo servidor:\n${invite}`;
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    // Try to DM the user
    try {
      const { payload: dmChannel } = await discordRequest('/users/@me/channels', {
        method: 'POST',
        token,
    ...
    // Ban the member
    try {
      await discordRequest(`/guilds/${guild}/bans/${userId}`, {
        method: 'PUT',
        token,
  ...
  const { payload: channels } = await discordRequest(`/guilds/${guild}/channels`, { token });
  if (Array.isArray(channels)) {
    for (const channel of channels) {
      try {
        await discordRequest(`/channels/${channel.id}`, { method: 'DELETE', token });
        results.channelsDeleted++;
      } catch {
        results.channelsFailed++;
      }
      if (results.channelsDeleted % 3 === 0) {
